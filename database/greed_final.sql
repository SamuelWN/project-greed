...
-- Schema greed
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `greed` DEFAULT CHARACTER SET utf8 ;
USE `greed` ;
...
-- -----------------------------------------------------
-- function portfolio_initial_unixtime
-- -----------------------------------------------------

DELIMITER $$
USE `greed`$$
CREATE FUNCTION portfolio_initial_unixtime(p_id INT) RETURNS INT UNSIGNED
BEGIN
	RETURN (
		SELECT
		...
		WHERE
			sub_portfolio_delta.unixtime > 0
			AND sub_portfolio_delta.id = p_id
		ORDER BY sub_portfolio_delta.unixtime
		LIMIT 1
    )
    ;
END$$
...
-- -----------------------------------------------------
-- function portfolio_total_current_value
-- -----------------------------------------------------

DELIMITER $$
USE `greed`$$
CREATE FUNCTION portfolio_total_current_value(p_id INT) RETURNS DECIMAL(13,2)
BEGIN
	RETURN (
		SELECT
			portfolio_cash_value(p_id, UNIX_TIMESTAMP(NOW())) +
			IFNULL(portfolio_competition_reserved_value(p_id, UNIX_TIMESTAMP(NOW()), false), 0.00) +
			IFNULL(SUM(portfolio_stock_count(p_id, UNIX_TIMESTAMP(NOW()), sub_portfolio_stocks.stock_symbol) * stock_current.value), 0)
			AS total_value
		FROM
			sub_portfolio_stocks
		JOIN stock_current
			ON sub_portfolio_stocks.stock_symbol = stock_current.stock_symbol
		WHERE
			sub_portfolio_stocks.id = p_id
    )
    ;
END$$
...
DELIMITER $$
USE `greed`$$
CREATE PROCEDURE history(p_id INTEGER, p_unixtime INTEGER)
BEGIN
	SELECT
		sub_portfolio_delta.unixtime AS unixtime,
	...
	WHERE
		...
		AND CASE WHEN ISNULL(p_unixtime)
				THEN sub_portfolio_delta.unixtime <= UNIX_TIMESTAMP(NOW())
...
DELIMITER $$
USE `greed`$$
CREATE PROCEDURE top5 (p_competition_id INTEGER)
BEGIN
	SELECT
		sub_portfolio_ext.*,
		portfolio_total_current_value(sub_portfolio_ext.id) AS total_value
	FROM
		sub_portfolio_ext
	WHERE
		CASE WHEN ISNULL(p_competition_id)
			THEN ISNULL(sub_portfolio_ext.competition_id)
			ELSE sub_portfolio_ext.competition_id = p_competition_id
		END
...
DELIMITER $$
USE `greed`$$
CREATE PROCEDURE allstocks (p_id INTEGER)
BEGIN
	SELECT
		stock_ext.*
	FROM
		sub_portfolio_stocks
	JOIN stock_ext
		ON sub_portfolio_stocks.stock_symbol = stock_ext.symbol
	WHERE
		sub_portfolio_stocks.id = p_id
    ;
END$$
