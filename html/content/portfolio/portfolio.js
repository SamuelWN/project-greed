...
function populateWhatif(symbol, count, unixtime, stockValues) {
	...
	chartData.push({"x":unixtime*1000, "y":0, "offsetStocks": initialOffset, "offsetCash": -1 * initialOffset});
	...
	for (var h in historyDates) {
		...
		if (date > unixtime) {
			...
			chartData.push({"x":date*1000, "y":offset - initialOffset, "offsetStocks": offset, "offsetCash": -1 * initialOffset});
		}
	}
	for (s in historyChart.series) {
		if (historyChart.series[s].name == "whatif") {
			historyChart.series[s].setData(chartData);
			historyChart.series[s].update({showInLegend: true});
		}
	}
}
